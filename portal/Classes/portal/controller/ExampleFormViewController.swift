...
class ExampleFormViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Return", style: .Plain, target: self, action:#selector(ExampleFormViewController.goback(_:)))
    }
    func goback(_: UIBarButtonItem!) {
        
       NSLog("goback")
        let secondStoryBoard:UIStoryboard = UIStoryboard(name: "Portal", bundle: nil)
       let secondVC:RAMAnimatedTabBarController = secondStoryBoard.instantiateViewControllerWithIdentifier("ram") as! RAMAnimatedTabBarController
